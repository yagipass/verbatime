...
The JMX flags from [Quick start](../README.md#quick-start) and `roots=` go in the same place. In a
container, leave out `jmxremote.host`, as these examples do. By default, everything except the JDK
and the agent is instrumented, including the server and the framework.
